/**
 * RAYO CERO — RACE OPERATIVE DETAIL (STABLE BUILD V11.3 - REGISTRATION LOCK)
 * Senior Dev: MIA (Valkyron Group)
 * CEO: Lualdo Sciscioli
 * Grado: Militar / Operativo / Diseñador
 * REGLA DE ORO: Evolución sin omisiones.
 *
 * CHANGELOG V11.3 (CIERRE DE INSCRIPCIONES):
 * [V11.3-1] Botón "INSCRIBIRME" bloqueado cuando INSCRIPCIONES_ABIERTAS = false.
 *           Muestra "CUPO COMPLETO" con estilo rojo. Navega a /registro
 *           (que renderiza el locked screen automáticamente).
 * [V11.3-2] Flag importado desde @/lib/registrationConfig — fuente única de verdad.
 * [V11.3-3] Toda la lógica del mapa, waypoints y panel lateral intacta.
 */
...
import {
  MapPin, Clock, Zap, Users, ArrowLeft, Trophy, Lock,
} from "lucide-react";
...
// Bypass TypeScript para Leaflet
...
/* ─────────────────────────────────────────────────────────────────────────── */
/* GENERADOR DE CLÚSTERES TÁCTICOS — idéntico al original                     */
/* ─────────────────────────────────────────────────────────────────────────── */
const createCompoundIcon = (waypoint: any) => {
  let poiHtml = "";

  if (waypoint.pois && waypoint.pois.length > 0) {
    const poiCircles = waypoint.pois
      .map(
        (poi: any) =>
...
/* ─────────────────────────────────────────────────────────────────────────── */
/* MAP CONTROLLER — idéntico al original                                       */
/* ─────────────────────────────────────────────────────────────────────────── */
...
const RaceDetail = () => {
  ...
  /* ── BOTÓN DE INSCRIPCIÓN — respeta flag ──
   * Siempre navega a /registro. Si las inscripciones están cerradas,
   * el RegistrationForm renderiza el locked screen automáticamente. */
  const handleRegistrationClick = (e: React.MouseEvent) => {
    e.preventDefault();
    navigate("/registro");
  };

  /* ─── RUTA ULTRA-PRECISA: NIGHT FEST 10K (BARQUISIMETO) — sin tocar ─── */
  ...
  /* ─── MATRIZ OPERATIVA (WAYPOINTS) — sin tocar ─── */
  const WAYPOINTS = [
    {
      pos: [10.077576, -69.283447], isMeta: true, label: "META",
      pois: [
        { icon: "♪", color: "#a855f7" },
        { icon: "B", color: "#94a3b8" },
        { icon: "+", color: "#22c55e" },
  ...
  return (
    <div className="h-screen w-full bg-[#03070b] flex flex-col lg:flex-row overflow-hidden text-white relative font-sans pt-[85px] md:pt-[104px] z-0">
      ...
      {/* ── INFO PANEL LATERAL — sin tocar excepto el botón ── */}
      <aside className="w-full lg:w-[35%] h-[55vh] lg:h-full overflow-y-auto bg-[#03070b] p-8 lg:p-12 custom-scrollbar border-l border-white/5 relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-12 pb-10">
          ...
          {/* ══ BOTÓN INSCRIPCIÓN — única modificación quirúrgica ══ */}
          {INSCRIPCIONES_ABIERTAS ? (
            ...
            <button
              onClick={handleRegistrationClick}
              className="w-full py-7 rounded-2xl font-black text-xs tracking-[0.4em] uppercase italic transition-all flex items-center justify-center gap-4 overflow-hidden active:scale-95"
              style={{
