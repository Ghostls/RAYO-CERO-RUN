/**
 * RAYO CERO — HOME CORE MODULE (STABLE V7.9.5 - MOBILE PERFORMANCE HUD)
 * Senior Dev: MIA (Valkyron Group)
 * CEO: Lualdo Sciscioli
 * Grado: Diseñador / Operativo
 * * EVOLUCIÓN V7.9.5 (GPU BOTTLENECK FIX):
 * - HARDWARE ACCELERATION: Inyección de `translateZ(0)` y `will-change` en Aurora Engine y Carrusel.
 * - ASYNC DECODING: Añadido `decoding="async"` a flyers y sponsors para liberar el Main Thread en móviles.
 * - ADAPTIVE BLUR: Reducción de carga poligonal en móviles (blur-80px vs blur-150px en desktop).
 * - TERMINAL DE ACCESO: Enrutamiento apuntando estrictamente a "/acceso" (AthleteAuth).
 * - REGLA DE ORO RESPETADA: Evolución estructural sin omisiones.
 */
...
// Mapeo dinámico secuencial (Sponsors)
...
const Index = () => {
  ...
  return (
    <div className="relative min-h-screen bg-[#03070b] flex flex-col overflow-x-hidden font-sans text-white z-0">
      ...
      {/* ─── EFECTO AURORA BOREAL (GPU OPTIMIZED PARA MÓVILES) ─── */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
        <motion.div
          style={{ willChange: "transform", transform: "translateZ(0)" }}
          animate={{ x: [0, 50, 0], y: [0, 30, 0] }}
          transition={{ duration: 25, repeat: Infinity, ease: "linear" }}
          className="absolute -top-[10%] -left-[10%] w-[60%] h-[50%] bg-[#50E8E3]/5 blur-[80px] md:blur-[150px] rounded-full mix-blend-screen"
        />
        <motion.div
          style={{ willChange: "transform", transform: "translateZ(0)" }}
          animate={{ x: [0, -50, 0], y: [0, -30, 0] }}
          transition={{ duration: 30, repeat: Infinity, ease: "linear" }}
          className="absolute top-[40%] -right-[10%] w-[70%] h-[60%] bg-blue-600/5 blur-[80px] md:blur-[150px] rounded-full mix-blend-screen"
        />
      </div>
      ...
      {/* ─── MÓDULO VISUAL SCALE: INYECCIÓN CINEMÁTICA DE FLYERS (ASYNC LOAD) ─── */}
      ...
      {/* ─── BRANDING / SPONSORS INFINITE CAROUSEL (HARDWARE ACCELERATED) ─── */}
        ...
        <div className="w-full max-w-7xl mx-auto px-6">
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-8 md:gap-16 mb-12 pb-8 border-b border-white/5">
            <div className="flex flex-col items-center gap-3">
              ...
              <img src={logoWeRun} alt="We Run 10K" loading="lazy" decoding="async" className="h-16 md:h-24 w-auto object-contain filter drop-shadow-[0_0_15px_rgba(80,232,227,0.25)]" />
            </div>
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              <img src={logoValkyron} alt="Valkyron Group" loading="lazy" decoding="async" className="h-10 md:h-14 w-auto object-contain opacity-80" />
            </div>
          </div>
          ...
          <div className="relative w-full overflow-hidden [mask-image:linear-gradient(to_right,transparent,white_15%,white_85%,transparent)]">
            ...
            >
              {infiniteSponsors.map((src, index) => (
                <div key={index} className="h-10 md:h-14 w-32 md:w-40 flex items-center justify-center shrink-0 opacity-40 grayscale hover:grayscale-0 hover:opacity-100 transition-all duration-500">
                  <img src={src} alt="Sponsor Oficial" loading="lazy" decoding="async" className="max-h-full max-w-full object-contain drop-shadow-md" />
                </div>
              ))}
            </motion.div>
          </div>
      ...
      {/* 2. MÓDULO DE CARRERAS */}
      <div className="relative z-10">
        <RacesSection />
      </div>

      {/* 3. MANIFIESTO Y MÉTRICAS */}
      <section className="py-24 px-6 max-w-7xl mx-auto w-full relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "50px" }} transition={{ duration: 0.5 }} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-[#03070b]/60 border border-white/5 p-8 rounded-[2rem] backdrop-blur-xl shadow-2xl">
            ...
            <div className="bg-[#03070b]/60 border border-white/5 p-8 rounded-[2rem] backdrop-blur-xl sm:col-span-2 shadow-2xl">
               ...
               <p className="text-[10px] font-bold text-white/40 tracking-widest uppercase max-w-xs">Circuitos blindados con seguridad operativa e hidratación.</p>
            </div>
          </motion.div>
      ...
      <section className="py-24 px-6 max-w-5xl mx-auto w-full relative z-10">
        ...
        <div className="text-center mb-16">
          <p className="text-[10px] font-black tracking-[0.5em] text-[#50E8E3] uppercase mb-4">Gestión Operativa</p>
        ...
        <div className="flex flex-col gap-6">
          ...
          <Link to="/registro" className="group">
            <motion.div whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} className="w-full bg-[#03070b]/60 border border-white/5 rounded-[2.5rem] p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 backdrop-blur-3xl hover:bg-[#50E8E3]/5 hover:border-[#50E8E3]/30 transition-all duration-500">
              <div className="flex items-center gap-6 md:gap-8 w-full md:w-auto text-center md:text-left">
                ...
                <div>
                  <h3 className="text-2xl md:text-3xl font-black italic text-white tracking-tighter mb-2">MÓDULO DE INSCRIPCIÓN</h3>
                  <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Portal de registro oficial y validación de atletas.</p>
                </div>
              </div>
          ...
          <Link to="/resultados" className="group">
            <motion.div whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} className="w-full bg-[#03070b]/60 border border-white/5 rounded-[2.5rem] p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 backdrop-blur-3xl hover:bg-[#50E8E3]/5 hover:border-[#50E8E3]/30 transition-all duration-500">
              <div className="flex items-center gap-6 md:gap-8 w-full md:w-auto text-center md:text-left">
                ...
                <div>
                  <h3 className="text-2xl md:text-3xl font-black italic text-white tracking-tighter mb-2">SISTEMA DE TIEMPOS</h3>
                  <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Cronometraje UHF y ranking en vivo.</p>
                </div>
              </div>
              <div className="h-12 w-12 rounded-full bg-white/5 border border-white/10 flex items-center justify-center group-hover:bg-[#50E8E3] group-hover:text-black transition-colors duration-500 shrink-0">
                <ArrowRight className="h-5 w-5" />
