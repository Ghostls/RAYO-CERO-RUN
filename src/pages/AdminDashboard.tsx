 ...
 *
 * CHANGELOG V4.0 (base sin modificaciones):
 * [V4-1..V4-8] Gender badge, scope bar guard, escuadrones scope, etc.
 *
 * CHANGELOG V3.0 (base sin modificaciones):
 * [V3-1..V3-4] AtletasList tabs, TasaConfig 4k, PDFExportModal, etc.
 *
 * CHANGELOG V2.1 (base sin modificaciones):
 * [V2.1-1] EscuadronesList join manual.
 */
...
/* [V4.2-1] Prefijos de carpeta en el bucket, ordenados por probabilidad */
...
const RaceScopeBar = ({ scope, onChange }: { scope: RaceScope | null; onChange: (s: RaceScope) => void }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      } catch (err) { console.error('[MIA] RaceScopeBar:', err); }
  ...
  return (
    <div className="mb-8 p-4 rounded-2xl bg-black/40 border border-white/10 flex flex-wrap items-center gap-3">
      ...
      {races.map(r => {
        ...
        const isCoro   = r.name.toLowerCase().includes('coro') || r.name.includes('499');
        const activeColor = isCoro ? 'bg-red-600 text-white border-red-600 shadow-lg shadow-red-600/20'
                                   : 'bg-cyan-500 text-black border-cyan-500 shadow-lg shadow-cyan-500/20';
        const archiveColor = isCoro ? 'bg-red-500/20 text-red-300 border-red-500/50'
        ...
        return (
          <button
            ...
            className={`px-5 py-2.5 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all flex items-center gap-2 border ${
              selected
                ? r.inscripciones_abiertas ? activeColor : archiveColor
                : 'bg-white/5 hover:bg-white/10 text-gray-400 border-white/10'
            }`}
          >
            {r.inscripciones_abiertas
              ? <span className={`w-1.5 h-1.5 rounded-full ${selected ? (isCoro ? 'bg-white' : 'bg-black') : 'bg-cyan-400 animate-pulse'}`} />
              : <Clock size={11} />
            }
            {r.name}
            {!r.inscripciones_abiertas && (
              <span className={`text-[8px] px-1.5 py-0.5 rounded ${selected ? 'bg-black/20' : 'bg-white/5'}`}>
                ARCHIVO
              </span>
            )}
          </button>
...
/* ────────────────────────────────────────────────────────────── */
/* [V4.2-1] COMPROBANTE URL — multi-layer resolver               */
/*                                                                */
/* Estrategia de resolución en 2 capas:                          */
/*  1. Prefijos de carpeta por carrera (scope-aware)              */
/*  2. Scan list() con paginación por cada prefijo               */
/*  3. Match flexible: teléfono primero, luego cédula, referencia */
/* ────────────────────────────────────────────────────────────── */

const tryGetPublicUrl = (path: string): string | null => {
...
const getComprobantePublicUrl = async (
  ...
  console.log('[MIA-COMPROBANTE] Iniciando búsqueda:', { cedula, referencia_pago, scopeName: scope?.name, telefono });

  // ── Prefijo prioritario según carrera ────────────────────────
  let scopePrefix = '';
  if (scope) {
    const nm = scope.name.toLowerCase();
    if (nm.includes('coro') || nm.includes('499')) scopePrefix = 'coro-499/';
    else if (nm.includes('lara') || scope.legacy)  scopePrefix = '';
    else scopePrefix = `${nm.replace(/\s+/g, '-').substring(0, 20)}/`;
  }

  const prefixOrder = [
    scopePrefix,
    ...STORAGE_PREFIXES.filter(p => p !== scopePrefix),
  ];

  // ── Patrones de match — teléfono primero ─────────────────────
  const cedulaRaw   = cedula.replace(/^V-?/i, '');
  const cedulaV     = `V${cedulaRaw}`;
  const cedulaVDash = `V-${cedulaRaw}`;
  const ref         = referencia_pago?.toLowerCase()?.trim() ?? '';
  const telefonoRaw = (telefono ?? '').replace(/\D/g, '');

  const matchesFile = (fileName: string): boolean => {
    const n = fileName.toLowerCase();
    if (telefonoRaw && n.startsWith(telefonoRaw)) return true;
    if (n.includes(cedulaRaw.toLowerCase())) return true;
    if (n.includes(cedulaV.toLowerCase()))   return true;
    if (n.includes(cedulaVDash.toLowerCase())) return true;
    if (ref && ref !== 'inscripcion_admin' && n.includes(ref)) return true;
    return false;
  };

  // ── Scan por prefijo con paginación ─────────────────────────
  for (const prefix of prefixOrder) {
    ...
    while (true) {
      const { data: files, error } = await supabase.storage
        .from(BUCKET)
        .list(prefix || '', { limit: PS, offset, sortBy: { column: 'name', order: 'asc' } });

      if (error) {
        console.warn(`[MIA-COMPROBANTE] Error listando prefijo "${prefix}":`, error.message);
        break;
      }
      if (!files || files.length === 0) {
        console.log(`[MIA-COMPROBANTE] No hay archivos en prefijo "${prefix}" (offset ${offset}).`);
        break;
      }

      console.log(`[MIA-COMPROBANTE] Listado prefijo "${prefix}" (${files.length} archivos). Nombres:`, files.map(f => f.name).slice(0, 20));

      const found = files.find(f => matchesFile(f.name));
      if (found) {
        const fullPath = prefix ? `${prefix}${found.name}` : found.name;
        const url = tryGetPublicUrl(fullPath);
        console.log(`[MIA-COMPROBANTE] ENCONTRADO: ${fullPath} → URL: ${url}`);
        if (url) return url;
        else console.warn('[MIA-COMPROBANTE] No se pudo generar URL pública.');
      }

      if (files.length < PS) break;
      offset += PS;
    }
  }

  console.warn('[MIA-COMPROBANTE] No se encontró archivo para:', { cedula, telefono: telefonoRaw, referencia_pago: ref });
  return null;
};

/* ────────────────────────────────────────────────────────────── */
/* PDF EXPORT — [V4.2-2] Coro 499 header + footer                */
/* ────────────────────────────────────────────────────────────── */
...
interface PDFExportModalProps {
  atletas: Runner[];
  raceName: string;
  scope: RaceScope;       // [V4.2-2] scope para detectar Coro 499
  onClose: () => void;
}
...
const generateCategoryPDF = (
  ...
  (doc as any).addPage = function (...a: any[]) {
    origAdd(...a);
  ...
  const accentG    = isCoro ? 38  : 211;
  const accentB    = isCoro ? 38  : 238;

  // Logo
  try {
    const img = new Image();
    img.src = logoPrincipal;
    doc.addImage(img, 'PNG', pageW / 2 - 20, 12, 40, 14);
  } catch {}

  // "RAYOCERO" — siempre en cyan
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(32);
  doc.setTextColor(34, 211, 238);
  doc.text('RAYOCERO', pageW / 2, 40, { align: 'center' });

  let headerBottomY: number;

  if (isCoro) {
    // [V4.2-2] "CORO 499" en rojo grande debajo de RAYOCERO
    doc.setFontSize(24);
    doc.setTextColor(220, 38, 38);
    doc.text(coroLabel, pageW / 2, 51, { align: 'center' });

    doc.setFontSize(9);
    doc.setTextColor(160, 160, 160);
    doc.setFont('helvetica', 'normal');
    doc.text(raceName.toUpperCase(), pageW / 2, 59, { align: 'center' });

    doc.setFontSize(8);
    doc.setTextColor(120, 120, 120);
    doc.text('Valkyron Group', pageW / 2, 65, { align: 'center' });
    headerBottomY = 70;
  } else {
    doc.setFontSize(12);
  ...
  // Línea divisora en color de la carrera
  ...
  doc.text(title, pageW / 2, headerBottomY + 10, { align: 'center' });
  ...
  doc.text(`Total: ${totalF}`, 14, headerBottomY + 18);
  doc.text(`Pagos verificados: ${pagF}`, 14, headerBottomY + 23);
  doc.text(`Generado: ${new Date().toLocaleString('es-VE')}`, pageW - 14, headerBottomY + 18, { align: 'right' });

  // [V4.2-2] Carrera en esquina derecha del header stat
  doc.setTextColor(accentR, accentG, accentB);
  doc.text(isCoro ? coroLabel : raceName.toUpperCase(), pageW - 14, headerBottomY + 23, { align: 'right' });

  // ── Barras de distribución por categoría ──
  let barY     = headerBottomY + 32;
  const maxC   = Math.max(...Object.values(categoryCounts), 1);
  const barMaxW = pageW - 28 - 40;
  const catsBar = [
  ...
  catsBar.forEach(cat => {
    ...
    doc.text(String(cnt), pageW - 14, barY + 3.8, { align: 'right' });
    doc.setFont('helvetica', 'normal');
    barY += 8;
  ...
  const toRender = [
    ...selectedCategories.filter(c => CATEGORY_ORDER.includes(c)),
    ...selectedCategories.filter(c => !CATEGORY_ORDER.includes(c)),
  ];
  toRender.sort((a, b) => {
  ...
  toRender.forEach(categoria => {
    ...
    doc.setFontSize(9);
    doc.text(`${ca.length} ATLETAS`, pageW - 14, 13, { align: 'right' });
    // [V4.2-2] Carrera en esquina izquierda de cada sección
    doc.setFontSize(7);
    doc.text(isCoro ? coroLabel : raceName.toUpperCase().substring(0, 18), 14, 13);
    ...
    autoTable(doc, {
      ...
      theme: 'plain',
      styles: {
        fontSize: 8, cellPadding: 3, textColor: [255, 255, 255],
        lineColor: [40, 40, 40], lineWidth: 0.1, fillColor: [5, 5, 5],
      },
  ...
  // ── [V4.2-2] Footer en todas las páginas ──
  const tp = (doc.internal as any).getNumberOfPages?.() ?? 1;
  for (let i = 1; i <= tp; i++) {
    doc.setPage(i);
    doc.setFontSize(6);
    doc.setTextColor(80, 80, 80);
    const footerText = isCoro
      ? `CORO 499 · RAYOCERO · Valkyron Group · Pág ${i}/${tp}`
      : `RAYOCERO · Valkyron Group · Pág ${i}/${tp}`;
    doc.text(footerText, pageW / 2, pageH - 5, { align: 'center' });
  }

  // ── Guardar ──
  const suffix    = mode === 'general' ? 'GENERAL'
    : mode === 'specific' ? selectedCategories[0]?.replace(/\s+/g, '_').toUpperCase()
    : 'SEGMENTADO';
  const raceSlug  = isCoro
    ? 'CORO499'
    : raceName.replace(/\s+/g, '_').toUpperCase().substring(0, 14);
  doc.save(`RAYOCERO_${raceSlug}_${suffix}_${Date.now()}.pdf`);
};

/* PDFExportModal — [V4.2-2] recibe scope */
const PDFExportModal: React.FC<PDFExportModalProps> = ({ atletas, raceName, scope, onClose }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (mode === 'segmented') cats = availableCategories;
        ...
        if (!selectedCategory) { alert('Selecciona una categoría.'); setIsGen(false); return; }
        cats = [selectedCategory];
      } else {
        cats = [...new Set(filteredAtletas.map(a => a.categoria || 'Sin categoría'))];
      }
      const label = modalidadFilter === 'todos' ? '' : ` (${modalidadFilter})`;
      generateCategoryPDF(filteredAtletas, cats, mode, raceName + label, scope);  // [V4.2-2]
    } catch (err: any) {
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <p className="text-[9px] text-gray-500 uppercase tracking-widest mt-1">
              {isCoroScope(scope) ? '🔴 CORO 499' : raceName} · Selecciona el modo
            </p>
          </div>
...
const ModuloEntregaKits = ({ scope }: { scope: RaceScope }) => {
  ...
  const searchRunner = async (e?: React.FormEvent) => {
    ...
    setStatusMsg({ text: 'Identidad confirmada. Escanear chip.', type: 'info' });
    ...
    <div className="max-w-4xl mx-auto bg-black/40 border border-cyan-500/20 rounded-2xl p-8 shadow-2xl relative overflow-hidden animate-in fade-in">
      ...
      <form onSubmit={searchRunner} className="flex gap-4 mb-8">
        <div className="flex-1">
          <label className="text-[10px] text-cyan-400 font-black uppercase tracking-widest mb-2 block">Número de Dorsal</label>
          <input type="number" value={bibInput} onChange={e => setBibInput(e.target.value)} disabled={activeRunner !== null}
            className="w-full rounded-xl bg-white/[0.03] border border-white/10 px-5 py-4 text-2xl font-black text-white outline-none focus:border-cyan-500/50 disabled:opacity-50" placeholder="0001" autoFocus />
        </div>
        <div className="flex items-end">
          <button type="submit" disabled={!bibInput || isLoading || activeRunner !== null}
            className="h-[64px] px-8 flex items-center justify-center gap-2 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-black font-black uppercase tracking-[0.2em] disabled:opacity-50">
        ...
        <div className="space-y-6 border-t border-white/10 pt-8 animate-in slide-in-from-bottom-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white/[0.02] border border-white/5 p-5 rounded-xl">
              <p className="text-[9px] text-gray-500 uppercase font-black tracking-widest mb-1">Objetivo</p>
            ...
            <div className="bg-white/[0.02] border border-white/5 p-5 rounded-xl flex flex-col justify-center">
              <div className="flex justify-between items-center mb-2">
                <span className="text-[9px] text-gray-500 uppercase font-black">Categoría:</span>
                <span className="text-xs text-white font-bold">{activeRunner.categoria || 'N/A'}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[9px] text-gray-500 uppercase font-black">Talla:</span>
          ...
          <div className="bg-cyan-900/10 border-2 border-dashed border-cyan-500/30 rounded-xl p-6 relative">
            ...
            <input ref={epcRef} type="text" value={epcInput} onChange={e => setEpcInput(e.target.value)} onKeyDown={handleEpcScan} disabled={isLoading}
              className="w-full bg-transparent border-b-2 border-cyan-500/50 pb-2 text-center text-xl font-mono text-white outline-none focus:border-cyan-400 placeholder-cyan-500/20"
              placeholder="[ APROXIME EL CHIP AL LECTOR ]" />
            <p className="text-center text-[9px] text-gray-500 uppercase mt-4">El sensor enlazará automáticamente.</p>
          </div>
          <div className="flex justify-end">
            <button onClick={() => { setActiveRunner(null); setBibInput(''); }} className="text-[10px] uppercase font-black tracking-widest text-red-500 hover:text-red-400">[ Abortar ]</button>
          </div>
        </div>
      )}
    </div>
...
const ModuloChequeoKits = ({ scope }: { scope: RaceScope }) => {
  ...
  const fetchStats = useCallback(async () => {
    let q = supabase.from('runners').select('kit_entregado');
    q = applyScopeFilter(q, scope);
    const { data } = await q;
    if (data) {
      const e = data.filter(r => r.kit_entregado).length;
      setStats({ total: data.length, entregados: e, pendientes: data.length - e });
    }
  ...
  const searchRunner = async (e?: React.FormEvent) => {
    ...
    setActiveRunner(data as Runner);
    if (data.kit_entregado) setStatusMsg({ text: `⚠️ El kit del Dorsal #${data.bib_number} ya fue entregado.`, type: 'error' });
    else setStatusMsg({ text: 'Atleta identificado. Confirme la entrega.', type: 'info' });
  };
  ...
  const confirmarEntrega = async () => {
    ...
    if (error) setStatusMsg({ text: `Error: ${error.message}`, type: 'error' });
      ...
      setStatusMsg({ text: `✅ KIT ENTREGADO — #${activeRunner.bib_number} ${activeRunner.nombre} ${activeRunner.apellido}`, type: 'success' });
      setActiveRunner(null); setBibInput(''); fetchStats();
      setTimeout(() => { setStatusMsg(null); bibRef.current?.focus(); }, 2500);
    }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in">
      ...
      <div className="bg-black/40 border border-amber-500/20 rounded-2xl p-8 relative overflow-hidden">
        ...
        <form onSubmit={searchRunner} className="flex gap-4 mb-6">
          <div className="flex-1">
            <label className="text-[10px] text-amber-400 font-black uppercase mb-2 block">Número de Dorsal</label>
            <input ref={bibRef} type="number" value={bibInput} onChange={e => setBibInput(e.target.value)}
              className="w-full rounded-xl bg-white/[0.03] border border-white/10 px-5 py-4 text-2xl font-black text-white outline-none focus:border-amber-500/50" placeholder="0001" autoFocus />
          </div>
          <div className="flex items-end">
            <button type="submit" disabled={!bibInput.trim() || isLoading}
              className="h-[64px] px-8 flex items-center justify-center gap-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-black uppercase tracking-[0.2em] disabled:opacity-50">
          ...
          <div className="border-t border-white/10 pt-6 space-y-4 animate-in slide-in-from-bottom-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-white/[0.02] border border-white/5 p-5 rounded-xl flex flex-col items-center justify-center">
                <p className="text-[9px] text-gray-500 uppercase font-black mb-1">Talla</p>
            ...
            <div className="flex gap-3">
              {!activeRunner.kit_entregado ? (
                <button onClick={confirmarEntrega} disabled={isLoading}
                  className="flex-1 py-4 rounded-xl bg-green-500 hover:bg-green-400 text-black font-black uppercase text-sm tracking-widest disabled:opacity-50 flex items-center justify-center gap-3">
                  {isLoading ? <RefreshCw size={18} className="animate-spin" /> : <CheckCircle size={18} />} Confirmar Entrega
                </button>
              ) : (
                <>
                  <div className="flex-1 py-4 rounded-xl bg-green-500/10 border border-green-500/30 text-green-400 font-black uppercase text-sm flex items-center justify-center gap-3">
                    <CheckCircle size={18} /> Ya Entregado
                  </div>
                  <button onClick={deshacerEntrega} disabled={isLoading}
                    className="px-6 py-4 rounded-xl bg-white/5 hover:bg-red-500/10 text-red-400 border border-red-500/20 font-black uppercase text-xs">
                    Revertir
                  </button>
                </>
              )}
              <button onClick={() => { setActiveRunner(null); setBibInput(''); setStatusMsg(null); }}
                className="px-6 py-4 rounded-xl bg-white/5 hover:bg-white/10 text-gray-400 font-black uppercase text-xs">
                Cancelar
              </button>
            </div>
          </div>
...
const AtletasList = ({ scope, onUpdateCount }: { scope: RaceScope; onUpdateCount?: (n: number) => void }) => {
  ...
  const [isAdminInscripcion, setIsAdminInscripcion] = useState(false); // [V4.2-1]
  ...
  const togglePago = async (id: string, current?: boolean) => {
    ...
    const nv = !current;
    try {
      const { error } = await supabase.from('runners').update({ pago_verificado: nv }).eq('id', id);
      if (error) throw error;
    ...
    finally { setIsTogglingPayment(null); }
  };
  ...
  const toggleKit = async (id: string, current?: boolean) => {
    ...
    const nv = !current;
    try {
      const { error } = await supabase.from('runners').update({ kit_entregado: nv }).eq('id', id);
      if (error) throw error;
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (error) throw error;
      setAtletas(prev => prev.filter(a => a.id !== tid));
      setAthleteToDelete(null);
    } catch (err: any) { alert(err.message); }
    finally { setIsDeleting(false); }
  };

  /* [V4.2-3] inspectComprobante — pasa scope y teléfono. Sin storedPath */
  const inspectComprobante = async (a: Runner) => {
    setSelectedAtleta(a);
    setComprobanteUrl(null);
    setImgLoading(true);
    setIsAdminInscripcion(false);

    // [V4.2-1] Inscripción admin no tiene comprobante en storage
    if (a.referencia_pago === 'INSCRIPCION_ADMIN') {
      setIsAdminInscripcion(true);
      setStatusMsg('Inscripción directa admin — sin comprobante de pago en storage.');
      setImgLoading(false);
      return;
    }

    setStatusMsg('Escaneando storage...');
    try {
      const url = await getComprobantePublicUrl(
        a.cedula,
        a.referencia_pago,
        scope,
        a.telefono,
      ...
      if (!url) {
        setStatusMsg(
          `No encontrado — V-${a.cedula}` +
          (a.referencia_pago ? ` / Ref: ${a.referencia_pago}` : '') +
          ` / Tel: ${a.telefono ?? '—'}` +
          ` / Carrera: ${scope.name}`
        );
      } else {
      ...
      setStatusMsg('Error de enlace con storage');
    } finally {
      setImgLoading(false);
    }
  };
  ...
  return (
    <div className="relative text-white">
          ...
          <div className="flex flex-wrap gap-1 mb-0">
            ...
            {countSinModalidad > 0 && (
              <div className="px-4 py-3 text-[9px] font-black uppercase tracking-widest text-amber-500/60 flex items-center gap-1.5 border-b-2 border-transparent">
                <AlertTriangle size={11} />{countSinModalidad} sin modalidad (solo en TODOS)
              </div>
            )}
            {countMismatch > 0 && (
              <div className="px-4 py-3 text-[9px] font-black uppercase tracking-widest flex items-center gap-1.5 border-b-2 border-transparent"
                ...
                <ShieldAlert size={11} />{countMismatch} conflicto{countMismatch > 1 ? 's' : ''} género/cat
              </div>
            )}
          </div>
        </div>
                ...
                <tr key={a.id} className={`hover:bg-white/[0.02] transition-colors ${validateGenderCategoryConsistency(a) ? 'bg-red-500/[0.02]' : ''}`}>
                  <td className="p-4">
                    <div className="font-bold text-xs uppercase flex items-center gap-2 flex-wrap">
                      ...
                      {/* [V4.2-1] Badge inscripción admin */}
                      {a.referencia_pago === 'INSCRIPCION_ADMIN' && (
      ...
      {selectedAtleta && createPortal(
        <div onClick={() => { setSelectedAtleta(null); setIsAdminInscripcion(false); }}
          className="fixed inset-0 z-[999999] flex items-center justify-center bg-black/95 p-4 backdrop-blur-sm">
          <div onClick={e => e.stopPropagation()}
            className="bg-[#0a0f14] border border-cyan-400/20 rounded-3xl w-full max-w-6xl overflow-hidden flex flex-col md:flex-row shadow-2xl animate-in zoom-in-95">

            {/* Panel izquierdo — comprobante */}
            <div className="flex-1 bg-black flex items-center justify-center p-4 min-h-[400px]">
              {imgLoading ? (
                <div className="text-center"><RefreshCw className="animate-spin text-cyan-400 mb-4" size={40} /><p className="text-cyan-400 text-xs uppercase">Buscando...</p></div>
              ) : isAdminInscripcion ? (
                /* [V4.2-1] Badge especial para inscripciones admin */
                <div className="text-center space-y-4">
                  ...
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-widest mb-2" style={{ color: '#818cf8' }}>
                      Inscripción Admin
                    </p>
                    <p className="text-gray-500 text-xs uppercase">Sin comprobante de pago</p>
                    <p className="text-gray-600 text-[9px] uppercase mt-1">Registro directo desde dashboard</p>
                  </div>
                </div>
              ) : comprobanteUrl ? (
                ...
                <div className="text-center">
                  <ShieldAlert size={60} className="text-gray-700 mb-4" />
                  <p className="text-gray-500 text-xs uppercase">{statusMsg}</p>
                </div>
              )}
            </div>

            {/* Panel derecho — datos del atleta */}
            <div className="w-full md:w-[350px] bg-[#0d1319] border-l border-white/5 p-8 flex flex-col">
              <div className="flex justify-between items-start mb-6">
                <div>
                  <h5 className="text-white font-black text-xl uppercase italic">Inspección</h5>
                  <p className="text-cyan-400 text-[9px] uppercase tracking-widest">
                    {isCoroScope(scope) ? '🔴 CORO 499' : scope.name}
                  </p>
                </div>
                <button onClick={() => { setSelectedAtleta(null); setIsAdminInscripcion(false); }} className="p-2 bg-white/5 rounded-full hover:bg-white/10"><X size={20} /></button>
              </div>

              {/* [V4-7] Alerta de mismatch */}
              ...
              <div className="space-y-3 overflow-y-auto flex-1">
                <div className="bg-white/[0.03] p-4 rounded-xl border border-white/5">
                  ...
                  <p className="text-white font-black text-sm uppercase flex justify-between">
                ...
                <div className="bg-white/[0.03] p-4 rounded-xl border border-white/5">
                  <p className="text-gray-500 text-[9px] uppercase">Categoría</p>
                  <p className="text-cyan-400 font-black uppercase">{selectedAtleta.categoria || 'SIN ASIGNAR'}</p>
                </div>
                <div className="bg-white/[0.03] p-4 rounded-xl border border-white/5">
                  <p className="text-gray-500 text-[9px] uppercase">Teléfono</p>
                  <p className="text-white font-mono">{selectedAtleta.telefono || 'SIN REGISTRO'}</p>
                </div>
                <div className="bg-white/[0.03] p-4 rounded-xl border border-white/5">
                  <p className="text-gray-500 text-[9px] uppercase">Talla</p>
                  <p className="text-white font-mono uppercase">{selectedAtleta.talla_camiseta || 'N/A'}</p>
                </div>
                <div className="bg-white/[0.03] p-4 rounded-xl border border-white/5">
                  <p className="text-gray-500 text-[9px] uppercase">Referencia</p>
                  <p className={`font-mono break-all text-xs ${selectedAtleta.referencia_pago === 'INSCRIPCION_ADMIN' ? 'text-indigo-400' : 'text-green-400'}`}>
                    {selectedAtleta.referencia_pago || 'PENDIENTE'}
                  </p>
                </div>
                ...
                <div className={`p-4 rounded-xl border flex items-center justify-between ${selectedAtleta.kit_entregado ? 'bg-amber-500/10 border-amber-500/30' : 'bg-white/[0.03] border-white/5'}`}>
                  <div>
                    ...
                    <p className={`font-black text-sm uppercase ${selectedAtleta.kit_entregado ? 'text-amber-400' : 'text-gray-500'}`}>{selectedAtleta.kit_entregado ? 'Entregado ✓' : 'Pendiente'}</p>
                  </div>
          ...
          <div className="bg-[#0a0a0a] border border-red-500/30 rounded-3xl p-8 max-w-md w-full shadow-2xl animate-in fade-in zoom-in-95">
            ...
            <div className="flex gap-4">
              ...
              <button onClick={handleDelete} disabled={isDeleting}
                className="flex-1 py-4 rounded-xl bg-red-600 hover:bg-red-500 flex items-center justify-center gap-2 font-black uppercase text-xs">
                ...
                {loading ? (
                  ...
                  <tr key={r.id} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="p-4">
                      ...
                      {r.repr_email && <p className="text-[9px] text-white/30 mt-0.5 font-mono truncate max-w-[160px]">{r.repr_email}</p>}
                    </td>
                    <td className="p-4">
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            <div className="p-8 grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div className="space-y-4">
                ...
                {[
                  ['Nombre completo', `${selected.nombre} ${selected.apellido}`, 'text-white'],
                  ['Cédula', `V-${selected.cedula}`, 'text-cyan-400 font-mono'],
                  ['Dorsal', selected.bib_number ? `#${selected.bib_number}` : 'Sin asignar', 'text-white text-2xl italic'],
                  ['Categoría', selected.categoria || '—', 'text-cyan-400 uppercase tracking-wide'],
                ].map(([lbl, val, cls]) => (
                  <div key={lbl} className="bg-white/[0.03] p-4 rounded-xl border border-white/5">
                    <p className="text-gray-400 text-[9px] uppercase mb-1">{lbl}</p>
                    <p className={`font-black ${cls}`}>{val}</p>
                  </div>
                ))}
                <div className="bg-white/[0.03] p-4 rounded-xl border border-white/5">
              ...
              <div className="space-y-4">
                ...
                {[
                  ['Nombre completo', `${selected.repr_nombre} ${selected.repr_apellido}`, 'text-white'],
                  ['Cédula', `V-${selected.repr_cedula}`, 'text-yellow-400 font-mono'],
                  ['Teléfono', selected.repr_telefono || '—', 'text-white font-mono'],
                  ['Relación con el atleta', selected.repr_relacion || '—', 'text-yellow-400'],
                ].map(([lbl, val, cls]) => (
                  <div key={lbl} className="p-4 rounded-xl border" style={{ background: 'rgba(251,191,36,0.04)', borderColor: 'rgba(251,191,36,0.15)' }}>
                    <p className="text-gray-400 text-[9px] uppercase mb-1">{lbl}</p>
                    <p className={`font-black uppercase ${cls}`}>{val}</p>
                  </div>
                ))}
                {selected.repr_email && (
                  <div className="p-4 rounded-xl border" style={{ background: 'rgba(251,191,36,0.04)', borderColor: 'rgba(251,191,36,0.15)' }}>
                    ...
                    <p className="text-white font-mono text-xs break-all font-black">{selected.repr_email}</p>
                  </div>
                )}
              </div>
            </div>
...
/* ────────────────────────────────────────────────────────────── */
/* ESCUADRONES LIST — [V4-4] scoped por carrera                   */
/* ────────────────────────────────────────────────────────────── */
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (scope.legacy) tq = tq.is('race_id', null);
        else if (scope.raceId) tq = tq.eq('race_id', scope.raceId);
        ...
        const { data: rd, error: re } = await supabase.from('runners').select('id,nombre,apellido,bib_number,telefono,talla_camiseta,categoria').in('id', ids);
        if (re) throw re;
        const bibs = (rd || []).map(r => r.bib_number).filter(Boolean);
        const { data: resd } = bibs.length > 0
        ...
        const proc = td.map(team => {
          ...
          const dm = mids.map(id => {
            const m = runMap.get(id);
            if (!m) return null;
            const s = parseTimeToSeconds(resMap.get((m as any).bib_number)?.tiempo_chip);
            if (s > 0) tot += s; else allF = false;
            return { nombre: `${m.nombre} ${m.apellido}`, bib: m.bib_number, telefono: m.telefono, talla_camiseta: m.talla_camiseta, categoria: m.categoria, tiempoStr: s > 0 ? formatSeconds(s) : 'EN PISTA', secs: s };
          }).filter(Boolean);
      ...
      {equipos.length === 0 ? (
        ...
        <div key={eq.id} className="bg-black/40 border border-white/10 rounded-2xl p-6 shadow-2xl relative overflow-hidden hover:border-cyan-500/30 transition-all">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 className="text-xl font-black text-white italic uppercase">{eq.team_name}</h3>
              <p className="text-[9px] text-gray-400 uppercase">Fuerza: {eq.members.length}/4</p>
            </div>
          </div>
          <div className="space-y-3 mb-6">
            {eq.members.map((m: any, j: number) => (
              <div key={j} className="flex justify-between items-center bg-white/[0.03] p-3 rounded-xl border border-white/5">
                <div>
                  <span className="text-xs font-bold text-white uppercase block">{m.nombre}</span>
                  <span className="text-[9px] text-gray-500 font-mono">#{m.bib || '---'} — {m.categoria || 'SIN CAT'} — {m.talla_camiseta || 'N/A'}</span>
                </div>
...
const PreRaceOverlay: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  useEffect(() => {
    intRef.current = setInterval(() => {
      setSeconds(p => {
        if (p <= 1) { clearInterval(intRef.current!); setFired(true); return 0; }
        return p - 1;
      });
    }, 1000);
  ...
  const glow  = `rgba(${r},${g},0,0.35)`;
  const radius = 140; const circ = 2 * Math.PI * radius; const dash = circ * (1 - seconds / 60);

  return createPortal(
    <div className="fixed inset-0 z-[9999999] flex flex-col items-center justify-center"
      style={{ background: fired ? 'rgba(0,255,80,0.08)' : 'rgba(0,0,0,0.96)', backdropFilter: 'blur(8px)', transition: 'background 0.4s' }}>
      <div className="absolute inset-0 pointer-events-none"
        style={{ background: `radial-gradient(ellipse at center, ${glow} 0%, transparent 65%)`, animation: seconds <= 10 && !fired ? 'prerace-pulse 0.5s ease infinite' : undefined }} />
        ...
        <>
          ...
          <div className="relative flex items-center justify-center mb-12">
            <svg width="320" height="320" style={{ transform: 'rotate(-90deg)' }}>
              ...
              <circle cx="160" cy="160" r={radius} fill="none" stroke={color} strokeWidth="8" strokeLinecap="round"
            ...
            <div className="absolute flex flex-col items-center">
              <span className="font-black tabular-nums leading-none"
                style={{ fontSize: '9rem', color, textShadow: `0 0 40px ${color}`, fontFamily: 'Barlow Condensed,sans-serif', fontStyle: 'italic', transition: 'color 0.5s' }}>
                {seconds}
              </span>
...
const TelemetryModule = ({ scope }: { scope: RaceScope }) => {
  ...
  useEffect(() => {
    if (raceState === 'running') {
      const tick = () => { setElapsedMs(Date.now() - startTs! + pausedMs); rafRef.current = requestAnimationFrame(tick); };
      rafRef.current = requestAnimationFrame(tick);
    }
    return () => { if (rafRef.current) cancelAnimationFrame(rafRef.current); };
  }, [raceState, startTs, pausedMs]);

  const fmt = (ms: number) => {
    const cs = Math.floor(ms / 10) % 100;
    const ts = Math.floor(ms / 1000);
    const s = ts % 60; const m = Math.floor(ts / 60) % 60; const h = Math.floor(ts / 3600);
  ...
  const handleFire = async () => {
    ...
    setGunFlash(true); setTimeout(() => setGunFlash(false), 600);
    setStartTs(now); setPausedMs(0); setElapsedMs(0); setRaceState('running'); setOfficialStart(iso);
    setSavingStart(true); setSaveMsg(null);
    try {
      let q = supabase.from('runners').update({ start_time: iso, race_status: 'in_progress' }).is('start_time', null);
      q = applyScopeFilter(q, scope);
      const { error } = await q;
      if (error) throw error;
      await supabase.from('race_signals').insert({ type: 'race_start', message: iso, created_by: 'admin', race_id: scope.raceId });
      setSaveMsg({ text: `✅ Pistola disparada · ${scope.name}`, ok: true });
    } catch (err: any) {
      setSaveMsg({ text: `⚠️ ${err.message}`, ok: false });
    } finally {
  ...
  const handlePR = () => {
    ...
    else if (raceState === 'paused') { setStartTs(Date.now()); setRaceState('running'); }
  };
    ...
    <>
      ...
      <div className="space-y-6 animate-in fade-in max-w-4xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-black italic uppercase text-white tracking-widest" style={{ fontFamily: 'Barlow Condensed,sans-serif' }}>Control de Carrera</h2>
            <p className="text-[9px] text-gray-500 uppercase mt-1">{scope.name}</p>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full" style={{ background: color, boxShadow: `0 0 8px ${color}`, animation: raceState === 'running' ? 'chrono-blink 1s ease infinite' : undefined }} />
            <span className="text-xs font-black uppercase" style={{ color }}>{SL[raceState]}</span>
          </div>
        </div>
        <div className="relative rounded-3xl overflow-hidden border"
          ...
          <div className="relative z-10 p-10 flex flex-col items-center">
            ...
            <div className="font-black tabular-nums leading-none mb-2"
              style={{ fontSize: 'clamp(5rem,14vw,9rem)', color, textShadow: raceState === 'running' ? `0 0 30px ${color},0 0 60px ${glow}` : 'none', fontFamily: 'Barlow Condensed,sans-serif', fontStyle: 'italic', animation: raceState === 'paused' ? 'chrono-blink 0.8s ease infinite' : undefined, transition: 'color 0.4s' }}>
              {fmt(elapsedMs)}
            </div>
            ...
            {saveMsg && (
              <div className={`mb-6 px-5 py-3 rounded-xl text-xs font-black uppercase border ${saveMsg.ok ? 'bg-green-500/10 border-green-500/30 text-green-400' : 'bg-yellow-500/10 border-yellow-500/30 text-yellow-400'}`}>
                {savingStart && <RefreshCw size={12} className="inline animate-spin mr-2" />}{saveMsg.text}
              </div>
            )}
            <div className="flex flex-wrap gap-3 justify-center">
              {raceState === 'idle' && (
                <button onClick={handleFire} disabled={savingStart}
                  className="px-10 py-5 rounded-2xl font-black uppercase tracking-widest text-black flex items-center gap-3 text-sm disabled:opacity-50"
                  style={{ background: 'linear-gradient(135deg,#22c55e,#16a34a)', boxShadow: '0 0 32px rgba(34,197,94,0.4)' }}>
                  {savingStart ? <RefreshCw size={20} className="animate-spin" /> : <Zap size={20} />} DISPARAR PISTOLA
                </button>
              )}
              {(raceState === 'running' || raceState === 'paused') && (
                <button onClick={handlePR}
                  className="px-8 py-4 rounded-2xl font-black uppercase tracking-widest flex items-center gap-3 text-sm border"
                  style={{ background: raceState === 'running' ? 'rgba(251,191,36,0.1)' : 'rgba(0,242,255,0.1)', borderColor: raceState === 'running' ? 'rgba(251,191,36,0.3)' : 'rgba(0,242,255,0.3)', color: raceState === 'running' ? '#fbbf24' : '#00f2ff' }}>
                  {raceState === 'running' ? '⏸ PAUSAR' : '▶ REANUDAR'}
                </button>
              )}
              {(raceState === 'running' || raceState === 'paused') && (
                <button onClick={handleFinish} className="px-8 py-4 rounded-2xl font-black uppercase tracking-widest flex items-center gap-3 text-sm border border-green-500/30 bg-green-500/10 text-green-400 hover:bg-green-500/20">
                  🏁 FINALIZAR
                </button>
              )}
              {raceState !== 'idle' && (
                <button onClick={() => setConfirmReset(true)} className="px-6 py-4 rounded-2xl font-black uppercase tracking-widest text-xs flex items-center gap-2 border border-red-500/20 bg-red-500/5 text-red-400 hover:bg-red-500/15">
        ...
        <div className="bg-black/40 border border-white/[0.08] rounded-2xl p-6">
          <div className="flex items-start gap-4 mb-5">
            ...
            <div>
              ...
              <p className="text-gray-500 text-[10px] uppercase mt-0.5">Countdown 60s en pantalla de atletas</p>
            </div>
          </div>
          ...
          <div className="fixed inset-0 z-[9999999] flex items-center justify-center bg-black/90 backdrop-blur-md p-4">
            <div className="bg-[#0a0a0a] border border-red-500/30 rounded-3xl p-8 max-w-md w-full shadow-2xl animate-in zoom-in-95">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={handleReset} className="flex-1 py-4 rounded-xl bg-red-600 hover:bg-red-500 font-black uppercase text-xs flex items-center justify-center gap-2">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[8px] text-gray-500 uppercase tracking-widest mt-1">
                Valkyron Group{raceScope ? ` · ${isCoroScope(raceScope) ? '🔴 CORO 499' : raceScope.name}` : ''}
              </p>
            </div>
          </div>
          <button className="flex items-center gap-2 text-[10px] uppercase text-gray-400 hover:text-white">
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        ...
        {!raceScope ? (
          ...
          <>
            {activeTab === 'overview' && (
              <>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div>
                    <div className="rounded-3xl p-8 text-black shadow-2xl relative overflow-hidden group"
                      style={isCoroScope(raceScope)
                        ? { background: 'linear-gradient(135deg,#dc2626,#991b1b)', boxShadow: '0 0 60px rgba(220,38,38,0.3)' }
                      ...
                      <h4 className="text-xs font-black uppercase">
                        Participación · {isCoroScope(raceScope) ? 'CORO 499' : raceScope.name}
                      </h4>
            ...
            {/* [V4.2-4] ModuloInscripcionAdmin recibe scope */}
            {activeTab === 'inscripcion'  && <ModuloInscripcionAdmin scope={raceScope} />}
