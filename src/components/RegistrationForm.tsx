/**
 * RAYO CERO — REGISTRATION TERMINAL (STABLE BUILD V19.4_DYNAMIC - VALKYRON VISION)
 * Senior Dev: MIA (Valkyron Group)
 * CEO: Lualdo Sciscioli
 * Grado: Militar / Operativo / Diseñador
 * REGLA DE ORO: Código completo sin omisiones. Base mantenida.
 * EVOLUCIÓN: Sincronización de Costo Dinámico desde HQ + Parche de Renderizado Ultra-Centrado.
 * FIX: Eliminación de "Black Render Error" y alineación milimétrica de ejes.
 */
...
const RegistrationForm = () => {
  ...
  const [registrationFee, setRegistrationFee] = useState<number>(40); // Valor dinámico (Inicia en 40 por protocolo)
  ...
  // ─── RADAR DE FINANZAS (ENLACE DIRECTO SUPABASE - COSTO + TASA) ───
  useEffect(() => {
      ...
      const fetchInternalFinance = async () => {
        try {
          ...
          if (data) {
            ...
            console.log(`[MIA TACTICAL] Sincronización Financiera: $${data.costo_usd} | Tasa: ${data.tasa_bcv} VES`);
          }
        } catch (err) {
  ...
  // ─── PROTOCOLO DE DESCARGA VISUAL PNG (PRECISIÓN V19.4) ───
  const handleDownloadPNG = async () => {
    if (bibRef.current === null) return;
    setIsExporting(true);
    
    // Retardo táctico para asegurar pintado de fuentes y texturas
    setTimeout(async () => {
      try {
        const dataUrl = await toPng(bibRef.current!, { 
          cacheBust: true, 
          quality: 1,
          pixelRatio: 3, // Alta densidad para impresión
          backgroundColor: '#03070b', // Parche "Anti-Negro"
          style: {
            transform: 'scale(1)',
          }
        });
        ...
        alert('Error en motor de renderizado. Use captura de pantalla.');
      } finally {
        ...
        <div className="space-y-6">
          <div className="bg-cyan-500/5 border border-cyan-500/20 rounded-2xl p-6 md:p-8 backdrop-blur-md relative overflow-hidden">
            ...
            <div className="mb-8 p-5 rounded-2xl bg-white/[0.02] border border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4 relative z-10 shadow-[0_0_20px_rgba(0,0,0,0.2)] hover:border-cyan-500/30 transition-all">
              <div className="flex items-center gap-4 w-full sm:w-auto">
                <div className="h-12 w-12 rounded-xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center text-cyan-400 shrink-0">
                  {isFetchingFinance ? <RefreshCw className="h-5 w-5 animate-spin" /> : <Shield className="h-5 w-5" />}
                </div>
              ...
              <div className="flex flex-col items-end shrink-0 w-full sm:w-auto text-center sm:text-right border-t sm:border-t-0 border-white/5 pt-4 sm:pt-0">
                <div className="text-4xl font-black italic tracking-tighter text-white drop-shadow-[0_0_15px_rgba(0,242,255,0.4)] leading-none">
                  ${registrationFee}<span className="text-cyan-400 text-xl">.00</span>
                </div>
                
                {isFetchingFinance ? (
                  <div className="flex items-center gap-2 mt-2 text-cyan-500/60 justify-center sm:justify-end">
                    <Loader2 className="h-3 w-3 animate-spin" />
                    <span className="text-[8px] font-black tracking-widest uppercase">Consultando...</span>
                  </div>
                ) : exchangeRate ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 relative z-10 text-[10px] font-bold uppercase tracking-widest leading-relaxed">
              <div><span className="block text-cyan-400/60 mb-1">Titular (Razón Social)</span><span className="text-white text-sm">Rayocero Eventos C.A</span></div>
              <div><span className="block text-cyan-400/60 mb-1">RIF / Documento</span><span className="text-white text-sm">J-505771710</span></div>
              <div><span className="block text-cyan-400/60 mb-1">Pago Móvil (Teléfono)</span><span className="text-white text-sm">0414-5643372</span></div>
              <div><span className="block text-cyan-400/60 mb-1">Cuenta Bancaria (BNC)</span><span className="text-white text-sm">0191-0060-0921-6016-9493</span></div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div className="relative col-span-1 sm:col-span-2">
              ...
              <input className={inputClass} placeholder="EJ: 458921 (ÚLTIMOS DÍGITOS)" value={form.referenciaPago} onChange={(e) => update("referenciaPago", e.target.value)} />
            </div>
            ...
            <div className="sm:col-span-2 mt-2">
               <label className="w-full border-2 border-dashed border-white/10 hover:border-cyan-500/50 bg-white/[0.02] rounded-2xl p-8 flex flex-col items-center justify-center transition-all cursor-pointer group relative overflow-hidden">
                  ...
                  {isCompressing ? <Loader2 className="h-8 w-8 text-cyan-400 animate-spin" /> : comprobanteFile ? <CheckCircle className="h-8 w-8 text-green-400" /> : <UploadCloud className="h-8 w-8 text-white/20 group-hover:text-cyan-400 transition-colors mb-3" />}
                  <span className="text-[10px] font-black text-white/40 uppercase tracking-[0.2em] group-hover:text-white transition-colors text-center">{comprobanteFile ? "Evidencia Lista" : "Haz clic para subir comprobante"}</span>
               </label>
            </div>
          </div>
        </div>
      ...
      content: (
        <div className="space-y-6">
          ...
          <div className="bg-white/[0.02] border border-white/10 p-6 rounded-2xl flex items-start gap-4 backdrop-blur-md">
            <input type="checkbox" checked={form.aceptaDeslinde} onChange={(e) => update("aceptaDeslinde", e.target.checked)} className="mt-1 h-5 w-5 rounded border-white/20 bg-white/5 text-cyan-500 focus:ring-cyan-500/50 cursor-pointer" />
            <p className="text-xs text-white/60 leading-relaxed font-medium">Confirmo que he leído y acepto el <button onClick={(e) => { e.preventDefault(); setShowDeslinde(true); }} className="text-cyan-400 font-bold underline decoration-cyan-400/30 hover:decoration-cyan-400 transition-all">Deslinde de Responsabilidad</button> y los términos operativos de RAYOCERO.</p>
          </div>
        </div>
      ),
    },
  ...
  if (submitted && bibNumber) {
    ...
    return (
      <section id="success-bib" className="relative min-h-screen pt-32 pb-24 px-4 bg-[#03070b] flex items-center justify-center font-sans overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto w-full relative z-10 flex flex-col items-center">
          <motion.div initial={{ scale: 0.95, opacity: 0, y: 30 }} animate={{ scale: 1, opacity: 1, y: 0 }} className="w-full flex flex-col items-center">
            ...
            {/* ── ÁREA DE CAPTURA VISUAL PNG (V19.4 FIXED ALIGNMENT) ── */}
            <div 
              ref={bibRef} 
            ...
            >
              ...
              {/* Bib Number — Centrado Absoluto Geométrico */}
              ...
              {/* Bloque Identidad — Centrado en barra inferior */}
            ...
            <p className="mt-10 text-[9px] text-white/20 font-black uppercase tracking-[0.5em] text-center">Acreditación Oficial Generada Correctamente.</p>
          </motion.div>
        </div>
      </section>
    );
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          ...
          <div className="bg-white/[0.02] border border-white/5 p-8 md:p-14 rounded-[2.5rem] backdrop-blur-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative overflow-hidden">
            ...
            <div className="flex flex-col-reverse sm:flex-row justify-between gap-4 mt-12 pt-8 border-t border-white/5">
              {step > 0 ? (
                <motion.button whileTap={{ scale: 0.95 }} onClick={(e) => { e.preventDefault(); setStep(step - 1); }} className="w-full sm:w-auto flex items-center justify-center gap-3 rounded-full bg-white/[0.03] hover:bg-white/10 px-8 py-5 text-[10px] font-black text-white uppercase border border-white/10 transition-all"><ArrowLeft className="h-4 w-4 text-cyan-400" /> ATRÁS</motion.button>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-[#03070b]/80 backdrop-blur-xl" onClick={() => setShowDeslinde(false)}>
            <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} onClick={(e) => e.stopPropagation()} className="bg-[#080b11] border border-white/10 p-10 md:p-14 rounded-[2.5rem] max-w-2xl w-full max-h-[85vh] overflow-y-auto shadow-2xl relative custom-scrollbar">
              ...
              <div className="text-sm text-white/50 space-y-6 font-medium leading-relaxed">
                ...
                <div className="p-5 rounded-2xl bg-white/[0.02] border-l-2 border-cyan-400 italic text-white/80">"Me encuentro en óptimas condiciones físicas y asumo total responsabilidad operativa y médica por mi participación en este evento."</div>
                <ul className="space-y-4">
                  ...
                  <li className="flex gap-4 items-start"><span className="text-cyan-400 font-black mt-1">02.</span><span>Aceptación irrevocable del sistema de cronometraje electrónico.</span></li>
                  <li className="flex gap-4 items-start"><span className="text-cyan-400 font-black mt-1">03.</span><span>Cesión de derechos de imagen para la plataforma operativa y promocional de RAYOCERO.</span></li>
                </ul>
              </div>
