 ...
 *
 * CHANGELOG V7 (CIERRE DE INSCRIPCIONES):
 * [V7-1] Botón "INSCRIBIRME" bloqueado cuando INSCRIPCIONES_ABIERTAS = false.
 *        Muestra "CUPO COMPLETO" con estilo rojo y navega a /registro
 *        (que renderiza el locked screen).
 * [V7-2] Badge de status en card usa REGISTRO_STATUS — ya no es hardcoded.
 * [V7-3] Flag importado desde @/lib/registrationConfig — fuente única de verdad.
 */

import { useState, useEffect } from "react";
import { motion } from "framer-motion";
import { MapPin, Calendar, Clock, Info, Zap, Loader2, Lock } from "lucide-react";
import { Link, useNavigate } from "react-router-dom";
import { supabase } from "@/lib/supabase";
import { INSCRIPCIONES_ABIERTAS, REGISTRO_STATUS } from "@/lib/registrationConfig";
...
const CardImageSlider = ({ image, alt, status }: { image: string; alt: string; status: string }) => {
  /* Si las inscripciones están cerradas, el badge siempre lo refleja */
  const displayStatus = INSCRIPCIONES_ABIERTAS ? (status || "Inscripciones Abiertas") : "Inscripciones Cerradas";
  const isClosed = !INSCRIPCIONES_ABIERTAS;
...
const RacesSection = () => {
  ...
  /* Botón de registro — respeta el flag */
  const handleRegistrationClick = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    /* Siempre navega a /registro:
       - Si INSCRIPCIONES_ABIERTAS = true  → muestra el formulario
       - Si INSCRIPCIONES_ABIERTAS = false → muestra el locked screen */
    navigate("/registro");
  };
            ...
            races.map((race, idx) => (
              ...
              >
                <CardImageSlider
                  ...
                  alt={race.name}
                  status={race.status || "Inscripciones Abiertas"}
                />
                ...
                <div className="p-8 flex flex-col flex-grow relative z-20">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-auto">
                    ...
                    {/* ── BOTÓN INSCRIPCIÓN — respeta flag ── */}
                    {INSCRIPCIONES_ABIERTAS ? (
                      ...
                      >
                        <Lock className="h-3 w-3" /> CUPO COMPLETO
                      </button>
                    )}
                  </div>
                </div>
